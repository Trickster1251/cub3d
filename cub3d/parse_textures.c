...
{
	(app->map_ptr.w_i == 1) ?
	(print_error("Sprite twice init")) : (app->map_ptr.w_i = 1);
...
{
	(app->map_ptr.e_i == 1) ?
	(print_error("Sprite twice init")) : (app->map_ptr.e_i = 1);
...
{
	(app->map_ptr.so_i == 1) ?
	(print_error("Sprite twice init")) : (app->map_ptr.so_i = 1);
...
{
	(app->map_ptr.s_i == 1) ?
	(print_error("Sprite twice init")) : (app->map_ptr.s_i = 1);
